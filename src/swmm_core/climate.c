...
//
{
    ...
    switch (k)
    {
      case 0: // Time series name
        ...
        if ( i < 0 ) return error_setInpError(project,ERR_NAME, tok[1]);
        ...
        if ( ntoks > 2 )
        {
            if ( *tok[2] != '*')
            {
                if ( !datetime_strToDate(tok[2], &aDate) )
                    return error_setInpError(project,ERR_DATETIME, tok[2]);
      ...
      case 2: // project->Wind speeds
        ...
        {
            if ( ntoks < 14 ) return error_setInpError(project,ERR_ITEMS, "");
      ...
      case 4:  // Areal Depletion Curve data
        ...
        if ( ntoks < 12 ) return error_setInpError(project,ERR_ITEMS, "");
        ...
        for (j=0; j<10; j++)
        {
            if ( !getDouble(tok[j+2], &y) || y < 0.0 || y > 1.0 )
                return error_setInpError(project,ERR_NUMBER, tok[j+2]);
...
//
{
    ...
    if ( k != TEMPERATURE_EVAP && ntoks < 2 )
        return error_setInpError(project,ERR_ITEMS, "");
    switch ( k )
    {
      case CONSTANT_EVAP:
...
////
    return error_setInpError(project,ERR_KEYWORD, tok[0]);
}
